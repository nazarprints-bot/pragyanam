        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 sm:gap-4 max-w-4xl mx-auto">
          {plans.map((plan, i) => (
            <motion.div key={plan.name} initial={{ opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.08 }}
              className={`relative bg-background rounded-xl p-4 sm:p-6 border transition-all duration-200 ${plan.popular ? "border-accent shadow-glow sm:scale-[1.02]" : "border-border hover:shadow-card"}`}>
              {plan.popular && (
              ...
              <div className="mb-3 sm:mb-5"><h3 className="text-[13px] sm:text-[15px] font-bold text-foreground">{plan.name}</h3></div>
              <div className="mb-1">
                <span className="text-2xl sm:text-3xl font-extrabold text-foreground tracking-tight">{plan.price}</span>
