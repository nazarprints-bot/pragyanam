...
const PricingSection = () => {
  ...
  const plans = [
    ...
    },
    {
      ...
      features: [t("pricing.f.fullAccess"), t("pricing.f.verification"), t("pricing.f.community"), t("pricing.f.noCompromise")],
    ...
    <section id="pricing" className="py-20 lg:py-28">
      <div className="max-w-6xl mx-auto px-5">
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="text-center mb-14">
          <p className="text-[12px] font-semibold text-accent uppercase tracking-widest mb-3">{t("pricing.label")}</p>
          <h2 className="text-3xl sm:text-4xl font-bold text-foreground tracking-tight mb-3">{t("pricing.title")}</h2>
        ...
        <div className="grid md:grid-cols-3 gap-4 max-w-4xl mx-auto">
          {plans.map((plan, i) => (
            <motion.div key={plan.name} initial={{ opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.08 }}
              className={`relative bg-background rounded-xl p-6 border transition-all duration-200 ${plan.popular ? "border-accent shadow-glow scale-[1.02]" : "border-border hover:shadow-card"}`}>
              {plan.popular && (
              ...
              <div className="mb-5"><h3 className="text-[15px] font-bold text-foreground">{plan.name}</h3></div>
              <div className="mb-1">
              ...
              <Link to="/auth">
                <Button className={`w-full h-10 text-[13px] font-semibold rounded-lg ${plan.popular ? "gradient-navy text-primary-foreground hover:opacity-90" : "bg-muted text-foreground hover:bg-muted/80"}`}>
                  {plan.cta}
                </Button>
              </Link>
            </motion.div>
